...
def main():
    ...
    net.addLink(root, sw1, cls=TCLink)
    net.addLink(root, sw2, cls=TCLink)
    net.addLink(root, sw3, cls=TCLink)
    ...
    net.build()
    net.addNAT(name='nat0', linkTo='sw1', ip='10.0.0.100').configDefault()
    ...
    for sv in svs:
        sv.cmd("docker build -f ./Dockerfile -t web-server .")
        sv.cmd("docker run --rm -d -p 80:5000 --name server1 web-server")
